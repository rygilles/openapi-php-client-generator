			...
			foreach ($resourceData['properties'] as $property) {
				...
				elseif (isset($property['type']) && isset($this->resourcesData[$property['type']])) {
					...
					if ($required) {
						...
						$callBody .= '(isset(' . $arrayContext . '[\'' . $property['name'] . '\']' . ') ? (' . $this->computeOperationResponsesMaker($typeTag, $classTypeName, $operation, $property['type'], false, $newTabs, $arrayContext . '[\'' . $property['name'] . '\']') . ') : null), ' . "\n";
					}
				} else {
					if ($isArrayResponse) {
						if ($required) {
