...
class Generator
{
	...
	protected function makeOutputDirectory()
	{
		if (file_exists($this->outputPath)) {
			if (!is_null($this->outputInterface)) {
				$this->outputInterface->writeln('<info>Output directory already created (' . (__DIR__ . DIRECTORY_SEPARATOR . $this->outputPath) . ')</info>');
			}
		} else {
			if (!is_null($this->outputInterface)) {
				$this->outputInterface->writeln('<info>Making output directory (' . (__DIR__ . DIRECTORY_SEPARATOR . $this->outputPath) . ')</info>');
			}
